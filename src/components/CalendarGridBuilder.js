...
export default class CalendarGridBuilder extends Component {
  ...
  constructor(props) {
    ...
    const numberOfDays = this.daysInCurrentMonth(
      this.props.year,
      this.props.month
    );
    const placeholderArray = this.makePlaceholderArray(startingDay);
    const daysArray = this.makeDaysArray(
      this.props.tipData,
      numberOfDays,
      startingDay,
      this.props.month,
      this.props.year,
      this.props.pickerFocusDate
    );
    this.state = {
      startingDay,
      numberOfDays,
  ...
  render() {
    return (
      ...
      >
        ...
        >
          {this.state.placeholderArray.map(placeholder => placeholder)}
          {this.state.daysArray.map(day => day)}
        </CalendarGrid>
      </Swipe>
    );
  }
